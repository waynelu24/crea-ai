...
if __name__ == "__main__":
	...
	# connect to mysql
	mysqlConnection = MySQLdb.connect('localhost', login, password, 'test')  # 'CREAKB'
		...
		while treeString != '':
			...
			for sentencePhraseRelTableEntry in sentencePhraseRelTableEntries:
				...
				if iamdebugging == True:
					print "@ get obj_phrase: objPhraseID = " + str(objPhraseID)
				...
				if iamdebugging == True:
					print "object = " + str(objPhraseID)
					print "target = " + str(targetPhraseID)
				...
				else: # multiple result from query, use the oldest inserted one
